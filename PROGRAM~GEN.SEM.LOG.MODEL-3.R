...
## r = 2, m = 2 ##
r = 5   ## change the values of r  and re-run the program ##
n = 1000
B = 1000
...
for(i in 1:N)
{
  cond.var.eps[i] = abs(1+t(c.vec)%*%X.vector[i,])
}

eps1 = c()
for(i in 1:N)
{
  eps1[i] = rcauchy(1,0,(cond.var.eps[i]))
}
...
## calculate Power_of_T2 for r = 2 , 3, 4, 5 ##
